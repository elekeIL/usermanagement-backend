...
UserSchema.statics.getAllUsers = function(
  ...
  const dateOfBirthQuery: any = {};
  if (startDate && endDate) {
    dateOfBirthQuery.dateOfBirth = {
      $gte: new Date(startDate),
      $lte: new Date(endDate),
    };
  } else if (startDate) {
    dateOfBirthQuery.dateOfBirth = { $gte: new Date(startDate) };
  } else if (endDate) {
    dateOfBirthQuery.dateOfBirth = { $lte: new Date(endDate) };
  }

  console.log('Constructed Query:', { deleted: false, ...dateOfBirthQuery });

  return (<IUserModel> this)
      .find({ deleted: false, ...dateOfBirthQuery })
      .select('-refreshToken')
